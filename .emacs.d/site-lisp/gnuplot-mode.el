...
(defvar gnuplot-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c C-t") 'gnuplot-tex)
...
;;;###autoload
(defun gnuplot-tex ()
  "For use with a LaTeX terminal. Compiles the gnuplot file then runs a
script to compile the resulting latex file."
  (interactive)
  (gnuplot-compile)
  (call-process "~/Code/texplot.sh" nil)
)
